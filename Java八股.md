...
JIT（即时编译，Just In Time）在运行时，将字节码编译成机器码。它重点优化多次调用和循环执行的代码，称为热点代码（Hotspot Code）。它使用到的技术有：方法内联、逃逸分析、循环展开等等。它有C1与C2两种类型，C1是用于客户端快速启动的轻量级优化，C2是用于服务器长期运行的重量级优化。
