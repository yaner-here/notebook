...
不要使用无界队列，否则任务积压过多会占用大量内存空间，频繁触发Full GC，导致服务不可用。
...
### 什么是LSM树？为什么NoSQL使用LSM树，而非B+树？

LSM树（Log Structured Merge Tree）是一种层级数据结构。第一层是平衡二叉树实现的Memtable，超过一定尺寸后写入到第二层的一个不可变的有序字符串表SSTable。更新操作会写入到最新的SSTable，删除操作会写入一个墓碑标记到最新的SSTable。后台线程会对上一层的SSTable做合并排序，得到下一层的SSTable。查找操作会从顶层开始依次查找Memtable和所有的SSTable。

B+树的读取效率高，写入效率低，因为可能要更新树上路径的所有节点，造成大量随机IO。
LSM树的读取效率低，写入效率高，因为所有的IO操作都是顺序写入。

### 什么是FTI？它和B树的区别是什么？

Factorial Tree Index是B树的改进版本，在每个节点引入了缓冲区。每次插入的时候都会写入缓冲区，如果缓冲区满了就下推，因此把若干次随机小写入变成了一次顺序大写入，避免B树每次插入都要进行$\log n$次磁盘IO。

### MongoDB的扩展性体现在哪些层面？

1. Replica：主从同步，读写分离，故障转移
2. Shard：分库分表
3. Balance：负载均衡，自动触发重分配，保证Shard负载均衡

## ElasticSearch

### 什么是倒排索引？MySQL的Full-text索引也是基于倒排索引实现的，为什么ElasticSearch更快？

正排索引是通过ID查找文档内容，倒排索引是把文档通过分词器拆成若干个关键词，通过关键词查找ID。

MySQL的联合索引必须要遵循最左匹配原则，但是业务前端传来的复杂的查询条件不一定能满足。ElasticSearch的策略是，针对每一个查询条件都访问对应的倒排索引，最后取交集。
...
有两种方案：双写、级联同步。
- **双写**：通用方案。优点是由于始终存在双写，因此可以随时回滚。缺点是时间周期长，需要改造业务。
	1. 将原库配置为主节点，新库配置为从节点，走BinLog同步。
	2. 改造业务代码，同时写入原库和新库，同时记录新库写入失败的日志，供后续补写。
	3. 抽取部分数据，检验原库和新库的数据一致性。
	4. 灰度切换，逐步增加新库的流量比例，直到达到`100%`为止。
- 级联同步：适用于迁移到云数据库，主要为了防止云环境和本地机房环境不一致，于是引入了本地机房的备库。优点是不需要改造业务。缺点是需要在业务低峰期牺牲可用性。
	1. 将原库配置为主节点，云库配置为从节点，走BinLog同步。
	2. 将云库配置为主节点，备库配置为从节点，走BinLog同步。
	3. 当原库、云库、备库三者数据一致时，将读流量切换到云库，然后在业务低峰期暂停写入，将写流量切换到云库，恢复写入。
	4. 如果需要回滚，同样将读流量切换到备库，然后在业务低峰期暂停写入，将写流量切换到备库，恢复写入。
...
- 写回策略（Write Back）：读取时如果命中缓存则直接返回，如果未命中则随机选择一个缓存槽用于替换，当前如果缓存槽有脏标记，则先写入数据库，然后读取数据库并写入缓存，最后撤销脏标记。写入时只更新缓存，打上脏标记，由后台线程异步写入数据库。
	- 不能保证强一致性，可能造成数据丢失。
	- 适合写多的场景，例如CPU缓存。
...
## 解释分布式ID及其相关实现的优缺点（UUID、SnowFlake）

分布式ID需要满足：**全局唯一、单调递增、安全（不包含敏感信息）、高性能、高可用**。

| 分布式ID名称          | 有/无状态     | 方案                                                                                                                 | 优点                                                           | 缺点                                                                                                                                         |
| ---------------- | --------- | ------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------ |
| 数据库自增主键          | 有状态，基于数据库 | 创建主键索引`PRIMARY KEY ("id")`，每次生成一个分布式ID时单调递增。                                                                       | 1. 实现简单                                                      | 1. 每次生成ID时，都会更新索引，CPU负载大<br>2. 存在数据库单点故障                                                                                                   |
| 数据库号段模式          | 有状态，基于数据库 | 记录每个号段的`id_max`/`step`/`type`/`version`，表示为业务`type`分配`[id_max, id_max + step]`区间内的号段，加载到内存里以供批量分配。`version`字段做CAS。 | 1. 更新索引次数更少，CPU负载更小                                          | 1. 存在数据库单点故障                                                                                                                               |
| Redis `INCR`原子自增 | 有状态，基于数据库 | Redis命令`INCR <key> 1`。                                                                                             | 1. 性能非常高                                                     | 1. Redis的持久化不能保证完全不丢数据                                                                                                                     |
| UUID（`v4`）       | 无状态，基于算法  | `uint128_t`值，纯随机数                                                                                                  | 1. 不会泄露任何商业敏感信息，因为不包含时间戳                                     | 1. 不能保证单调递增。<br>2. （UUID不适合作为数据库主键的原因）                                                                                                     |
| SnowFlake        | 无状态，基于算法  | `int64_t`值，时间戳、机房ID、主机ID、自增序列号，单机`1ms`可产生`4096`个唯一ID。                                                              | （**所有五个优点**）                                                 | 1. 如果单机`1ms`的请求书超过`4096`，则只能阻塞到下一毫秒，需要改造自增序列号的`bit`数。<br>2. 时钟回拨问题。系统时间不一定准确。<br>3. 强依赖机器ID，必须引入中心化组件，否则无法处理集群的收缩扩容的场景。<br>4. 时间戳会泄露商业敏感信息 |
| UUID（`v7`）       | 无状态       | `uint128_t`值，时间戳、自增序列号或随机数（`74bit`）                                                                                | 1. 删除了主机ID，因此不需要引入中心化组件，使用自增序列号就能唯一指定一次请求，`74bit`造成冲突的概率非常小。 | 1. 不能保证`1ms`内的有序性<br>2. 时间戳会泄露商业敏感信息                                                                                                       |

SnowFlake时钟回拨问题：NTP延迟、硬件时钟漂移都有可能导致操作系统时间戳不准确。现在的解决方案有：
- 拒绝服务。适合对可用性要求不高的环境。
- 等待追回。一直原地挂机，直到当前超过了上一次生成的分布式ID的时间戳为止，从而保证单调递增。适合NTP内网环境，要求时钟误差仅有几毫秒。
- 切换备用主机ID。向ZooKeeper/Etcd/Redis预先申请一个备用主机ID保存在本地，一旦发现时钟回拨问题，就立即切换主机ID，从而保证单调递增。适合生产环境，但是要防范注册中心脑裂。

现代的企业级解决方案（例如`TinyID`、`Leaf`），还会增加一些新特性：
- 双Buffer：使用号段模式时，快要耗尽时，提前异步请求数据库分配新号段。
- 解决时钟回拨问题：使用ZooKeeper/Etcd/Redis，切换备用主机ID。
...
### 在微博信息流推送系统中，推模式和拉模式的优缺点是什么？

- 推模式：博主将消息发送到所有粉丝的收件箱中。优点是粉丝读取速度非常快，缺点是粉丝数增多时发送耗时很大，占用数据库存储空间，很难实现关注列表分组功能。
- 拉模式：粉丝自行向博主们拉取并聚合消息。优点是彻底解决了发送耗时的问题，降低了数据库存储空间，扩展性很好。缺点是聚合耗时很大，加重数据库的负担，需要引入缓存。
...
\begin{align}
	\text{TF-IDF}(q, d, D) 
	& = \sum_{t\in q} \bigg(词频\text{TF}(t, d) \times 逆文档频率\text{IDF}(t, D) \bigg) \\
	& = \sum_{t\in q} \left(\displaystyle\frac{词t在d中出现的次数}{d的总词数} \times \log\left(\frac{N}{包含词t的文档数}\right)\right)
\end{align}
$$

其中TF表示局部相关性，IDF表示全局区分性，两者结合就能检索出相关的、有区分性的文档。
...
$$
\begin{align}
	\text{BM25} & = \sum_{t\in q} \bigg(
		平滑词频(t, d)\times 平滑逆文档频率(t, D)
	\bigg) \\
	& = \sum_{t\in q} \left(
		\frac{(1+k)\cdot\text{TF}(t,d)}{\text{TF}(t,d) + k\cdot\left(
			1-b + b\cdot\frac{|D|}{\mathrm{avg}(|D|)}
		\right)}
		\times
		\ln \left(1 + \frac{N - 包含词t的文档数 + 0.5}{包含词t的文档数 + 0.5}\right)
	\right)
\end{align}
$$

平滑指的是同一个词出现的次数过多时，相关性不应该无限地增大，还应该考虑文档长度的影响。其中$k$越大表示高频词更占优；$b$越大表示短文档更占优。

### 什么是IVF（IVF_FLAT）？

IVF（Inverted File，倒排文件）先对向量进行KMeans聚类构建IVF索引，然后给定一个查询向量$q$，第一步对聚类中心逐个计算相似度取Top-K，第二步通过倒排索引获取每个聚类的向量，第三步对这些向量逐个计算相似度取Top-K即可。
...
- `efConstruction`：更新索引时每个节点维护的最近邻节点的数量。
...
为什么要切Chunk：
1. LLM的上下文窗口长度有限。
2. 希望精准召回最相关的段落，防止其它无关的信息误导LLM。

切Chunk的整体原则是：Chunk长度`256<=x<=1024`落在Embedding的甜点区间内，设置`>=10%`的重叠防止关键信息被截断，每个Chunk要附带Metadata用于过滤。
- Chunk长度
	- 太小会破坏语义信息，太大会降低检索精度。
	- 2024年的Embedding模型上下文长度只有`512`，到2026年（例如`Qwen-3.5-Embedding`）提升到了`32K`。所以只要厂商能保证召回精度不下降，那么我觉得Chunk的长度上限可以进一步提高。
- Chunk切块
	- **固定窗口切块**：实现最简单，但是很可能会断章取义。例如“该保险条款适用于医保报销场景，但以下不可抗力因素除外”，此时固定窗口切块就会造成严重的误导。
	- **滑动窗口切块**：相邻两个Chunk之间存在交集。优点是适用于聊天记录、播客采访的这种明显关联上下文的场景，缺点是会增加存储量。
	- **结构切块**：利用结构化文档中的标记作为分隔符。例如Markdown有`#`章节标记，HTML有`</>`语义标签，代码有`def/function/class`定义类和函数的关键字。优点是能自然的保留语义信息，缺点是必须要求是结构化文档，而且单个章节的长度不可控。
	- **结构递归切块**：在结构切块的基础上，如果发现单个章节的长度太长，就递归地切块。章节太长就按照`\n`拆成段落，段落太长就按照`。`切成句子。
	- **语义切块**：使用LLM搭建一个Pipeline用于分块。有的Pipeline是计算相邻句子之间的语义相似度，如果相似度突然下降就切成一个Chunk；还有Pipeline是让LLM每次插入若干个分块标记，迭代次数越多Chunk就切的越小；还有的不用Pipeline，直接训练一个LLM做端到端处理。优点是最能保留完整的语义信息，缺点是计算开销会更大。
	- **模态感知切块**：主动检测多模态信息的类别，对症下药。例如对于表格——表格太长就按行切Chunk，每个Chunk都要携带表头信息；表格太宽就按列切Chunk，每个Chunk都要携带主键信息。再例如对于图片，通常会扔到VLM里作Caption，然后把Caption和图像本身一起拼成一个Chunk。
	- 现在社区已经有很多成熟的解决方案了，比如LangChain预置了很多`Splitter`类，Spring AI也集成了Apache Tika框架。这些解决方案往往会把以上策略结合起来，达成更好的效果。
- Metadata
...
### 什么是Agent？与LLM有什么不同之处？与Workflow有什么不同之处？
...
传统的LLM有四个问题：知识被冻结、无法行动、无状态、无法自主纠错。
...
Workflow本是上是一个确定性的流程图，LLM只是用于执行其中某个节点的任务，不参与到决策中。
- 优点：行为完全可预测，容易测试和排查，适合流程非常固定的业务。
- 缺点：灵活性差，需要实现列举出所有的条件分支，这时才推荐使用Agent。

### Agent兴起的前提条件是什么？为什么2025年才火起来？
...
- 基础层面：LLM基础模型、记忆(长短期)、工具、文本指令(Prompt/Skills)
- 产品层面：消息网关Gateway(跨机器调用/钉钉企微邮箱接入)、Hook、Plugins及其应用商店
- 工程层面：沙盒、Session持久化、Trace可观测性、长短期记忆的存储选型、运维监控Dashboard
- 业务层面：测评(Rubric)、Agent Teams、自进化Evolve
...
3. 交给子Agent做校验，最后人工Code Review。
4. 把重复的流程沉淀成可复用的Skill。例如开发一个Feature之后，我需要把它涉及的上下游、设计思路写到部门的知识库里面做存档。在开发了很多Feature之后，我意识到我可以写一个Skill来自动化地根据代码变更来生成知识库存档。
...
每次请求时携带的上下文`message[]`有四部分：System Prompt、历史消息、当前用户输入、可用工具列表。每次更新MCP Tools列表时，只需更改可用工具列表即可，可以理解为栈顶元素用完就扔。

### 远程Skills仓库发生了更新，如何同步到本地Agent？
...
Hermes会创建后台任务自动回顾历史对话记录——判断其是否有价值，然后创建Skills；同时会检测失败点，然后更i性能Skills。这个过程成为KEPA（Knowledge-Enhanced Prompt Adaption）反向传播。

优点：
- 渐进式积累：随时使用时间的增长，Agent会越来越聪明。
- 可解释性强：Skills是自然语言，能够方便的读懂，方便的迁移到其它Agent。

缺点：
- 后台任务使用的LLM决定了Agent的能力上线。它可能分不清`重要的经验`和`不重要的噪声`，所以Skills的质量参差不齐。
...
3. **会话记忆压缩**：直接接使用已经积累的会话记忆作为摘要。这种压缩是渐进式的，不会像传统压缩那样一次性替换所有消息。
...
### 相比于单Agent，Agent Teams/MultiAgent的优点是什么？

Agent Teams是指多个Agent协同完成一个任务，并且每个Agent都有自己的分工，可以通过中心化的主Agent负责整合结果，也可以通过去中心化的消息队列或共享状态做HandOff。

不使用单Agent，而是使用Agent Teams的原因和优点：
- 不再受制于单个Agent上下文窗口的长度限制，Agent之间可以方便的做HandOff。
- 每个Agent都能发挥自己最擅长的技能，实现专业分工。
- 多Agent可以并行执行。
因此能处理更复杂的长程任务。
...
短期记忆：
- 内存：用于单机测试场景
- 文件 & Redis：使用`grep`或键值对，当场召回字符串。单用户量不成问题，`1MB`空间即可容纳1M字符；

长期记忆：
- ElasticSearch & Neo4j：倒排索引`GIN`+`BM25` + 图数据库做图搜索
- 向量数据库：`原文文本`+`摘要文本`+`Metadata(Tag、时间等等)`+`原文向量`+`摘要向量(容易召回)`
- 分布式存储：用于亿级用户量，需要用到分布式存储，目前主流的方案是`Redis混合数据库`与`LevelDB磁盘数据库`混合使用。
...
对于单个Session：
- `messages[-n:]`滑动窗口 + Memory数据库召回。
- 超过上下文阈值时做被动压缩，及时对篇幅较长的部分（例如工具返回结果）进行上下文卸载做主动压缩。
...
- 工作流：N8N、Dify、LangGraph
- ReAct：Spring AI Alibaba
- Agent：DeepAgents、AgentScope、OpenCode、Pi
- Team：Multica、CrewAI
- SDK：OpenAI Agents SDK、Qoder Agent SDK
- 阿里内部：
	- iFlow-agent(类似于DeepAgents)、LangHub(类似于LangChain)、IdeaLab(类似于Dify)。
...
- 一个Chain会包含以下环节：首先是`PromptTemplate`节点做模板替换构建Prompt，如果涉及到RAG则还有`Retriever`节点，输入到`Model`节点中请求LLM，最后交给`OutputParser`节点做格式化输出。这中间还可以增加一些`Runnable`或`RunnableParallel`执行自定义流程。
- LangChain社区之前对`可观测性`、`Flux流式响应`这些功能的支持也很不理想。
...
- RLHF（人类反馈强化学习，Erinforcement Learning from Human Feedback）：收集人类对于若干回答的喜好排名，用这些数据训练一个奖励模型来预测人类的偏好，最后用这个奖励模型来训练大模型。优点是让模型学会承认不确定性，提高安全性和人性化程度，缺点是需要大规模人工高质量标注，训练流程长、成本高，不容易收敛，容易出现Reward Hacking问题。
- DPO（直接偏好优化，Direct Perference Optimization）：收集人类对于若干回答的喜好排名，直接微调大模型，更偏向于输出偏好的答案，依赖于分类损失。在SFT和RLHF之间取得了平衡。
...
读博所考虑的课题其实更注重Idea的创建性。我在实习过程中做过一些Agent项目，发现一个Agent应用开发如果要进入到生产的环境的话，其实单单只有Idea是远远不够的，还需要保证高QPS的稳定性这些工程的难题。我在这一过程中意识到了学术界和工程界是有脱节的。我其实更倾向于把一个Agent项目应用到实际的业务中，去应对真实世界的挑战，所以我放弃了读博这条路。
...
- 日常实习：我在前司投递的是日常实习岗位，没有转正机会。那段时间导师又接了个横向，让我尽快回去写本子。
- 暑期实习：我原先的计划就是一直实习到转正答辩的时候，总共四个月的时间。答辩结束后就回到了学校，忙毕业论文的东西。
...
- **暑期实习**：我对Base地没有明确的要求，主要还是追求转正。

### 转正答辩怎么样？公司给你发Offer/谈薪了吗？
