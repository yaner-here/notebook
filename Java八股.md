...
| 小林Coding | 图解Redis-前言-图解Redis介绍                | √   |
| 小林Coding | 图解Redis-基础篇-什么是Redis                | √   |
| 小林Coding | 图解Redis-数据类型篇-Redis数据结构             | √   |
| 小林Coding | 图解Redis-数据类型篇-Redis常见数据类型和应用场景      | √   |
| 小林Coding | 图解Redis-持久化篇-AOF持久化是怎么实现的           | √   |
| 小林Coding | 图解Redis-持久化篇-RDB快照是怎么实现的            | √   |
| 小林Coding | 图解Redis-持久划篇-Redis大Key对持久化有什么影响     | √   |
| 小林Coding | 图解Redis-功能篇-多节点争抢资源，Redis分布式锁是怎么实现的 | √   |
| 小林Coding | 图解Redis-高可用篇-如何保证Redis分布式所的高可用和高性能  | √   |
| 小林Coding | 图解Redis-缓存篇-数据库和缓存如何保证一致性           | √   |
...
Memcached只支持最基本的数据存储，而Redis支持更多的高级功能，例如高级数据结构、快照、集群模式、事务、Lua脚本。

### 介绍Redis的`String`数据类型

`String`使用的数据结构是简单动态字符串（SDS），包含三部分——剩余空间长度`free`、字符串长度`len`、字符数组`buf[]`，最大可达`512MB`。相比于C语言的字符串，SDS的优点是——可以保存二进制数据、获取字符串长度的时间复杂度是$O(1)$，避免了拼接字符串的缓冲区溢出问题。

`String`使用三种编码：
- `int`：如果`String`是一个`long`范围内的整数，则直接存到`(long*) buf`。
- `embstr`：如果`String`长度较短，则通过一个`malloc()`同时分配`RedisObject`与`SDS`。缺点是只能作为只读字符串，如果要更改则会先从`embstr`转换为`raw`。
- `raw`：如果`String`长度较长，则通过两次`malloc()`分别分配`RedisObject`与`SDS`。

### 介绍Redis的`List`数据类型

`List`使用的数据结构是`QuickList`。缺点是：不支持多个消费者消费同一条消息，也不支持消费组，而`Stream`做到了这一点。

压缩列表本质上是一个`char[]`，将不同长度的元素按照编码字段排在一起，每个节点包含一个`prevlen`字段表示前一个元素占用的空间，用于实现逆序遍历。但是元素扩容时会导致后续的元素都要重新为`prevlen`字段扩充内存空间，导致连锁更新。

`QuickList`是压缩列表+双向链表的折中版本，双向链表的每个节点存储一个压缩列表，通过分块的思想控制压缩列表连锁更新的影响范围。
...
1. 扩容时给第2个哈希表分配两倍的空间
2. 每次针对第1个哈希表做增删改查时，会把对应的条目移动到第2个哈希表中。
3. 随着处理请求的增多，最终会在某个时间点全部清空第1个哈希表，从而完成Rehash操作
优点：可以把一次性大量的数据移动操作，平摊到多次处理请求的过程，避免了阻塞的情况。
...
跳表是一种多层的有序链表。每个跳表节点都有一个层级数量，表示含有多少个`next`指针。层级越高的指针跳跃举例越长。增删改查都是从高层指针依次向底层指针做二分查找。最好情况下相邻两层节点的数量比例为`2:1`能达到$O(\log n)$的时间复杂度，最坏情况下退化成链表$O(n)$的时间复杂度。为了保证这个数量比例，我们让新创建的跳表节点层级数量符合$p=0.25$的几何分布，且规定它的上限为`64`。
