...
- Agent
	...
	- [知乎 - 深度解析：一张图拆解OpenClaw的Agent核心设计](https://zhuanlan.zhihu.com/p/2004505448938767308)
...
Redis集群使用Gossip协议的`ping-pong`心跳机制进行检测。主节点向从节点发送`ping`命令确认从节点是否存活，从节点发送`pong`命令返回自己的赋值偏移量。
...
- 尽量保证网络连接的质量，加快数据同步。
- 监控Redis的数据同步进度，如果大于阈值则禁止读取从节点。

### 在Redis主从模式中，如何避免主从切换导致的数据丢失？

主从的数据同步是一个异步的过程，因此无法避免数据丢失。

对于数据同步来说，如果主节点发现同步延迟高于阈值，则会有风险。对于脑裂来说，如果主节点发现同步延迟高于阈值，或从节点短时间内大量下线，也会有风险。因此我们可以牺牲可用性，拒绝一切写入操作，从而减少数据丢失量。这需要程序FailBack降级到本地缓存或MQ。

### 在Redis集群模式中，客户端如何知道自己应该访问哪个切片？如果实例上没有对应的数据会发生什么？

客户端计算Key的`crc16()%16384`，向对应的Redis实例发送请求。

Redis实例检查自己是否负责这个哈希槽，如果不负责则返回`MOVED`重定向；如果负责这个哈希槽，但是未持有这个键值对，就检查该哈希槽是否正在迁出；如果正在迁出，则检查哈希槽是否正在导入；如果正在导入且有`ASKING`标记则直接操作，如果没有导入则返回`MOVED`重定向。
...
传统的Prompt工程由于要覆盖所有边界情况，往往会产生上下文腐败的问题。Skill把Prompt与外部数据源封装成一个文件夹，代表着一个可以按需加载的SOP，在处理简单任务时控制上下文长度，在处理复杂任务时瞬时加载大量的Prompt。
...
OpenClaw是一个Agent的运行时环境，支持会话管理、并发控制、记忆检索等功能。Gateway会监听外部的Channel事件（例如飞书、Telegram），把消息发送到队列中。
...
Cursor会记录每一**行**通过TAB补全或Agent生成的代码签名，然后读取Git提交记录来逐行匹配签名，从而统计AI生成的代码量。这里的单位是**行**，因此使用Formatter会干扰Cursor的统计功能。
