...
解决方法：
1. 设置`sql_safe_updates = 1`。它会要求`UPDATE`语句满足任意条件：使用`where`且命中索引；使用`limit`；使用`where`和`limit`。会要求`DELETE`语句舒勇`where`和`limit`。
...
MySQL：
- 小林Coding
	1. 图解MySQL - 事务篇 - MySQL可重复读隔离级别，完全解决幻读了吗
	2. 图解MySQL - 锁篇 - MySQL有哪些锁
	3. 图解MySQL - 锁篇 - MySQL是怎么加锁的
	4. 图解MySQL - 锁篇 - update没加索引会锁全表
	5. 图解MySQL - 锁篇 - MySQL记录锁+间隙锁可以防止删除操作而导致的幻读吗
	6. 图解MySQL - 锁篇 - MySQL 死锁了，怎么办
	7. 图解MySQL - 锁篇 - 字节面试：加了什么锁，导致死锁的
