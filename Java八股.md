...
其中词频表示局部相关性，IDF表示全局区分能力。所以TF-IDF能突出重要的、有区分性的词。
...
\begin{align}
	...
	\bigg) \\
	& = \sum_{t\in q} \left(
		\frac{(1+k_1)\cdot\text{TF}(t,d)}{\text{TF}(t,d) + k_1\cdot\left(
...
其中$k_1$越大，表示高频词得到更多的奖励；$b$越大，表示短文档更占优。BM25认为同一个词的出现次数过多时，相关性不应该无限地增大，还应该考虑文档长度的影响。

### 什么是IVF（IVF_FLAT）？

使用了倒排索引的思想。需要事先对向量进行KMeans聚类，给定一个查询向量$q$，首先检索距离它最近的若干个聚类中心，然后通过倒排索引获取对应的向量列表，计算得到若干个距离最近的向量即可。

有两个重要的参数：
- `nlist`：建立索引时KMeans聚类的类别数量
- `nprobe`：检索时要考虑的距离最小的类别数量。

### 什么是HNSW？

HNSW（Hierarchical Navigable Small World graphs，分层可导航小世界图）是一种基于图的近似最近邻（ANN）算法。它构建了一个多层图，最下层包含所有数据点，逐级采样得到上层。搜索时从顶层的一个固定数据点开始，在每一层都贪心的移动到最近邻居，直到遇到局部最小值位置，然后跳转到下层，重复这一步骤。
