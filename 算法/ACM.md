...
众所周知，在 `algorithm` 库中有一个 `max` 函数，但你不知道的是 `ISO c++11` 中 `max` 可以接受多个参数，前提是要用花括号括起来，如：
...
相比于传统线段树而言，zkw线段树是一种基于差分思想的、码量较小、空间常数减半（从`N_MAX*4`减为`N_MAX*2`）、时间常数小（从递归改成循环）的改良版线段树。
...
2. 令维护的序列范围为`a[1->n]`，长度为`n`，开辟一个长度为$2n$的线段树节点数组。设$m=2^{\lceil\log_{2}n\rceil}$,令`segtree[0]`不使用，`segtree[1->m]`这`m`个节点用于储存区间合并后的节点信息，`segtree[m+1->m+n]`这`n`个节点用于存储原始序列`a[0->n-1]`。**网上的大部分实现都是这一版，然而真用的时候，`segtree[m]`在`segtree_pushup()`时调用左右节点必然内存越界，必须至少开$1+2^{\lceil\log_{2}{n}\rceil}+n\le 4n$个空间才行**。所以网上大部分实现都额外预留了，**而且访问的越界节点还要注意初始化，不保证默认值就是正确的初始值，因此千万不要使用**。\
...
。**普通线段树维护的区间为`a[1->n]`，为了方便讨论，我们令zkw线段树维护的区间为`a[0->n-1]`**。zkw线段树将所有叶子节点都放在最后一层，分别对应着`a[0->n-1]`的`n`个值（允许右边的叶子节点用不到）。于是满二叉树最后一层的节点数量必为$n_{\text{offset}}=2^{\lceil\log_{2}{n}\rceil}$，原序列第`i`个节点`a[i]`在zkw线段树的节点编号为$i+n_{\text{offset}}$，**总共需要$O(2^{\lceil\log_{2}n\rceil})$**。
