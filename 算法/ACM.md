...
这里我们要介绍一个关于字典序的引理：如果已知`dp[i-1][j-weight[i]][k-friction[i]]`对应的最佳方案，且`dp[i][j][k] == dp[i-1][j-weight[i]][k-friction[i]]`，则`dp[i][j][k]`的最佳方案一定是保持前`i-1`个物品的选择策略不变，再额外选择第`i`个物品。使用反证法：

- 如果不选择第`i`个物品，那么最佳方案就变成了`dp[i-1][j][k]`的方案，然而选择第`i`个物品能显著提升总价值，因此矛盾。所以必须选择第`i`个物品。
- 如果前`i-1`个物品的选择策略发生变化，则记变化前后的选择策略对应的“单词”分别为`s1`和`s2`，第`i`个物品的选择策略对应的单词为`a[i]`。要比较
...
- 当`dp[j][k] == dp[j - weight[i]][k - friction[i]] + value[i]`时，有可能选择第`i`个物品。例如下面这组输入样例：
  ...
  重量代价：1 阻力代价：1 价值：1
  ```
  容易发现`1 3`和`2`这两种物品选择方案都能让总价值最大化，但是题目要求输出字典序最小的`1 3`。这要求我们在遍历到`i=3`，也就是第3个物品时，注意到存在一个更靠前的已选1号物品。形式化地，如果存在一个已选`l`号物品（即$\exist l\in[1, i$），使得`is_selected[l][j][k] && !is_selected[l][j-weight[i]][k-friction[i]]`成立，那么说明在总价值相同的情况下，选择第`i`个物品会使得第`l`个物品不能被选择，使得字典序增大，因此不能选择第`i`个物品；如果使得`!is_selected[l][j][k] && is_selected[l][j-weight[i]][k-friction[i]]`成立，则`!is_selected[l][j-weight[i]][k-friction[i]]`说明在给定前`i`个物品时，最佳方案包含第第`i`个物品，但不包含第`l`个物品，选择第`l`个物品无法使得总价值最大。
